...
const WindStationMarker: React.FC<{ item: any; focusedId: number | null }> = ({
  item,
  focusedId,
}) => {
  ...
  const handleOpenPopup = () => {
    setLoading(true);
    axios
      .get(`/wind/api/v1/stations/${item.id}/`)
      .then((res) => {
  ...
  const val =
    item.wind_speed !== undefined && item.wind_speed !== null
      ? parseFloat(item.wind_speed)
      : 0;
      ...
      <Popup autoPan={false}>
        <div style={{ fontSize: "11px", minWidth: "180px", padding: "2px" }}>
          ...
          >
            ...
            >
              {item.name || `Station #${item.id}`} ({item.station_code}){" "}
              <i
          ...
          <strong>Cao độ:</strong>{" "}
          {item.elevation ? `${item.elevation}m` : "N/A"}
          <br />
          <strong>Tọa độ:</strong> {(item.lat || item.latitude || 0).toFixed(4)}
          , {(item.lon || item.longitude || 0).toFixed(4)}
          <br />
          {loading ? (
            <div style={{ marginTop: "5px", color: "#64748b" }}>
              <i className="fa fa-spinner fa-spin"></i> Đang tải đo đạc mới
              nhất...
            </div>
            ...
            >
              ...
              <div>
                Thời gian:{" "}
                {new Date(
                  detailData.properties.latest_observation.obs_time,
                ).toLocaleString("vi-VN")}
              </div>
              <div>
                Nhiệt độ:{" "}
                {detailData.properties.latest_observation.temp_2m !== null
                  ? `${detailData.properties.latest_observation.temp_2m} °C`
                  : "N/A"}
              </div>
              <div>
                Tốc độ gió:{" "}
                {detailData.properties.latest_observation.wind_speed !== null
                  ? `${detailData.properties.latest_observation.wind_speed} m/s`
                  : "N/A"}
              </div>
              <div>
                Hướng gió:{" "}
                {detailData.properties.latest_observation.wind_dir !== null
                  ? `${detailData.properties.latest_observation.wind_dir}°`
                  : "N/A"}
              </div>
              <div>
                Độ ẩm:{" "}
                {detailData.properties.latest_observation.humidity !== null
                  ? `${detailData.properties.latest_observation.humidity}%`
                  : "N/A"}
              </div>
              <div>
                Khí áp:{" "}
                {detailData.properties.latest_observation.pressure !== null
                  ? `${detailData.properties.latest_observation.pressure} hPa`
                  : "N/A"}
              </div>
            </div>
          ) : (
            ...
            >
              Không có dữ liệu đo đạc mới nhất
            </div>
          )}
        </div>
      </Popup>
...
export const StationClusterLayer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (showStations) {
      axios
        .get("/wind/api/v1/stations/")
        .then((res) => {
          // serializer trả về GeoJSON dạng FeatureCollection, hỗ trợ phân trang
          const features =
            res.data.results?.features || res.data.features || [];
          const parsedStations = features.map((f: any) => ({
            ...
            wind_speed:
              f.properties.latest_observation?.wind_speed !== undefined &&
              f.properties.latest_observation?.wind_speed !== null
                ? parseFloat(f.properties.latest_observation.wind_speed)
                : 0,
            wind_dir:
              f.properties.latest_observation?.wind_dir !== undefined &&
              f.properties.latest_observation?.wind_dir !== null
                ? parseFloat(f.properties.latest_observation.wind_dir)
                : 180,
          }));
